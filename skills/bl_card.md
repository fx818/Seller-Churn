...
version: "1.0"
category: messaging
description: Aggregate all skill outputs into a single seller briefing card (BL Card).
python_class: bl_card
...
## Purpose
The **final aggregator skill** — runs last in the pipeline and consumes outputs from
every other skill via `flow.*` sources. Produces a unified seller briefing card that
reps can use directly or paste into CRM.

## What It Outputs

A single structured "BL Card" containing:

| Section | Content |
|---------|---------|
| **header**       | GLID, company, location, customer type, verdict, priority |
| **scores**       | churn score, risk tier, LLM risk, BL/LMS/Activity bands |
| **root_cause**   | RCA category, explanations (Hindi + English), intervention |
| **signals**      | Churn reasons, trajectory, demand, peer comparison |
| **action_plan**  | Opening line, suggested actions, do-not-mention list |
| **messaging**    | WhatsApp + 5-part call script (Hindi + English) |
| **interventions**| BL upgrade eligibility, winback score + pitch |
| **lookalikes**   | Churned + retained seller GLIDs |
| **summary_text** | Plain-text printable card for CRM |

## How It Works

1. Runs as the **final phase** in `pipeline.md`
2. Reads from `flow.*` — the accumulated state from all prior skills
3. Computes a **priority score (0-100)** to help queue reps
4. Generates a **verdict**: CRITICAL / AT RISK / HEALTHY
5. Renders a **plain-text card** for printing or CRM paste

## CLI

```bash
python -m churn_analysis skill bl_card 11282573 --pretty
```

Note: when run standalone (not via pipeline), `flow.*` fields will be empty — the card
will only show snapshot context. Run via `pipeline` for the full card.

## Example Verdict Logic

- `Red risk OR churn_score >= 70` → **CRITICAL — Immediate retention call**
- `Amber risk OR churn_score >= 40` → **AT RISK — Schedule within 7 days**
- `LLM risk in [Critical, High]`     → **AT RISK — LLM-flagged, prioritize**
- Otherwise                          → **HEALTHY — Routine check-in**
